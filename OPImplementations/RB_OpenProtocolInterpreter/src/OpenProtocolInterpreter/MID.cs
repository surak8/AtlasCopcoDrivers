	...
	public abstract class Mid : IMid {
		...
		protected string GetValue(DataField field, string package) {
			try {
				...
				System.Diagnostics.Trace.WriteLine(this.exceptionValue(MethodBase.GetCurrentMethod(), aore));
				return null;
			}
		}

		string exceptionValue(MethodBase mb, Exception ex) {
			StringBuilder sb = new StringBuilder();
			Exception ex0=ex;

			sb.AppendLine(mb.ReflectedType.Name + "." + mb.Name + ":");
			while (ex0 != null) {
