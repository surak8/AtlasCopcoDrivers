...
namespace NSAtlasCopcoBreech {
	internal delegate void FieldGenerationHandler(int midNo,string fieldName,Type fieldType);

	public partial class MainWindow {
		public IColtLogger logger { get; private set; }
...
#else
			new TestGenerator(this.logger).generateTestsFor(asm,new FieldGenerationHandler(generateSpecificField));
#endif
		}

		void generateSpecificField(int midNo,string fieldName,Type fieldType) {
			//throw new NotImplementedException();
			this.logger.log(ColtLogLevel.Debug,MethodBase.GetCurrentMethod(),"MID="+midNo+", field='"+
				fieldName+", Type="+fieldType.FullName+".");
		}
	}
	class TestGenerator {
		...
		internal TestGenerator(IColtLogger icl) {
			this.logger=icl;
		...
		internal void generateTestsFor(Assembly asm,FieldGenerationHandler fgh) {
			object obj;
			CodeMemberMethod amethod;
			string aname;

			this._fieldGenHandler=fgh;
		...
		static void showCompileUnit(CodeCompileUnit ccu,CodeDomProvider provider) {
			...
			string currentPath = Directory.GetCurrentDirectory(), newFilePath;
			newFilePath=Path.Combine(currentPath,"..\\..","Source\\Tests");
			if (!Directory.Exists(newFilePath))
				Directory.CreateDirectory(newFilePath);
			File.WriteAllText(Path.GetFullPath(Path.Combine(newFilePath,"Tester."+provider.FileExtension)),sb.ToString());
		}
		...
		FieldGenerationHandler _fieldGenHandler;
		...
		CodeStatementCollection generateCTORS(Type midType,CodeVariableReferenceExpression vr,CodeVariableReferenceExpression vrPkg,string setPropMethodName,int revision,CodeTypeMember ctm,CodeDomProvider cdp) {
			...
			foreach (ConstructorInfo cc in midType.GetConstructors())
				addCTOR(cc,ret,vr,midType,testStatements,revision,ctm);
			return ret;
		}
		...
		void addCTOR(ConstructorInfo cc,CodeStatementCollection ret,CodeVariableReferenceExpression vr,
			Type midType,CodeStatementCollection testStatements,int revision,CodeTypeMember ctm) {
			ParameterInfo[] parms;
	...
	internal class TraceLogger : IColtLogger {
		...
		void IColtLogger.log(ColtLogLevel error,MethodBase mb,string v) {
			throw new NotImplementedException();
		}
